...
But now, it's also now possible to do the same example but with a completely dynamic schedule. With the improved language runtime, this takes a few small tweaks now. We use the new `\verb|@spawn|' macro with the existing `\verb|@sync|' macro to delineate the work items. The `\verb|@spawn|' macro marks a block of code that can immediately start executing, asynchronously, on any free thread. The preexisting `\verb|@sync|' macro then waits for all (lexical) subtasks to complete, eliminating the boilerplate necessary to track and wait on each task block separately.
